...
public:
	...
	void operator-=(GestiuneExamene examen) {
		...
		if (ok == 1) {
			GestiuneExamene* aux = new GestiuneExamene[nrExamene - duplicate];
			for (int i = 0; i < nrExamene; i++) {
				if (!(examene[i] == examen)) {
					aux[i] = examene[i];
				}
	...
	friend ostream& operator<< (ostream& out, const Catalog& catalog) {
		out << "\n\nCATALOG\n\n";
		out << catalog.nrExamene << endl;
		for (int i = 0; i < catalog.nrExamene; i++) {
			out << catalog.examene[i] << endl;
		}
...
//template
template <class T> class CatalogTemplate {
	T* examene;
	int nrExamene;
public:

	CatalogTemplate(T* examene, int nrExamene) {
		this->nrExamene = nrExamene;
		this->examene = new T[nrExamene];
		for (int i = 0; i < nrExamene; i++) {
			this->examene[i] = examene[i];
		}
	}

	CatalogTemplate(const Catalog& c) {
		this->nrExamene = c.nrExamene;
		this->examene = new T[c.nrExamene];
		for (int i = 0; i < c.nrExamene; i++) {
			this->examene[i] = c.examene[i];
		}
	}
	...
	void operator-=(T examen) {
		...
		if (ok == 1) {
			T* aux = new T[nrExamene - duplicate];
			for (int i = 0; i < nrExamene; i++) {
				if (!(examene[i] == examen)) {
					aux[i] = examene[i];
				}
	...
	friend ostream& operator<< (ostream& out, const CatalogTemplate& catalog) {
		out << "\n\nCATALOG\n\n";
		out << catalog.nrExamene << endl;
		for (int i = 0; i < catalog.nrExamene; i++) {
			out << catalog.examene[i] << endl;
...
void main() {
	...
	//-= +=
	Catalog catalog(new GestiuneExamene[3]{ g3,g4,g5 }, 3);
	cout << catalog;
	catalog += g5;
	cout << catalog;
	catalog -= g5;
	...
	cout << "\n\ntemplate\n\n";
	CatalogTemplate<GestiuneExamene> catalogTemplate(new GestiuneExamene[2]{ g3,g5 }, 2);
	cout << catalogTemplate;
}